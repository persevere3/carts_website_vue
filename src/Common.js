  ...
  mounted(){
    ...
    document.addEventListener("webkitAnimationStart", (event)=> {
      if (event.animationName === "nodeInserted") {
        ...
        let lineStyle;
        if (line) lineStyle = window.getComputedStyle(line);

        if(parents(chat, 'widget-visible')){
          let chatStyle = window.getComputedStyle(chat);

          // 
          // if(line) chat.style.bottom = parseInt(chatStyle.bottom) + 70 + 'px';

          // 
          // if(parseInt(chatStyle['right']) === 20 && parseInt(chatStyle['bottom']) === 20){
          //   line.style.bottom = parseInt(line.style.bottom) + 70 + 'px';
          // }
        }
        else if (chat.classList.contains('fb_dialog')){
          // fb 放在最下面
          // if(line) line.style.bottom = parseInt(lineStyle.bottom) + 70 + 'px';
          // let tawk = document.querySelector('.widget-visible iframe');
          // if(tawk) {
          //   let tawkStyle = window.getComputedStyle(tawk);
          //   tawk.style.bottom = parseInt(tawkStyle.bottom) + 70 + 'px';
          // }
